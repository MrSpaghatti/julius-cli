...
export const gitProvider = {
	...
	exec(args: string[], options: Record<string, unknown> = {}): string {
...
export function getCurrentBranch(): string {
	try {
		...
		// Branch may not exist — fall back to default
	}
	return "main";
}
...
export function inferRepo(): RepoInfo {
	...
	const providerRegex =
		/(?:(?:https?|git|ssh):\/\/|git@)(github\.com|gitlab\.com|bitbucket\.org)[/:]([^/]+\/[^\s/]+?)(?:\.git)?\/?$/;
	...
	throw new InvalidArgsError(
		`Could not parse repository path from URL: ${remoteUrl}`,
	);
}
...
export function pullSessionChanges(repo: string, branchName: string): void {
	try {
		...
		const remote = "origin";
		Output.info(
			`Fetching branch ${branchName} from ${repo} (remote: ${remote})...`,
		);
