...
export function App(): React.ReactNode {
	...
	const fetchSessions = useCallback(async () => {
		try {
			...
			if (repoFilter) {
				const parts = repoFilter.includes("/")
					? repoFilter
					: `github/${repoFilter}`;
	...
	useInput((input, key) => {
		...
		const stateIndex = FILTER_STATES.findIndex(
			(s) => s[0] === input && s !== "all",
		);
	...
	const filteredSessions = searchQuery
		? sessions.filter((s) => {
				...
				const matchesRepo = extractRepo(s.sourceContext)
					.toLowerCase()
					.includes(q);
				return matchesId || matchesTitle || matchesRepo;
			})
		: sessions;
	...
	return (
		<Box flexDirection="column" height="100%">
			...
			<Box borderStyle="single" paddingX={1}>
				...
				<Box marginLeft={2}>
					<Text>Filter: </Text>
					<Text bold color={filterState === "all" ? "green" : "white"}>
						{filterState}
					</Text>
				</Box>
			</Box>
				...
				<Box paddingX={1} paddingY={0}>
					...
					<Box flexGrow={1}>
						<TextInput
							...
							placeholder="repo, title, or ID (Esc to cancel)"
						/>
					</Box>
				</Box>
			)}
			...
			<Box flexGrow={1} minHeight={15}>
				...
				<Box width="55%" borderStyle="single" flexDirection="column">
					{!selectedSession ? (
						...
						>
							<Text color="gray">
								{loading ? "Loading..." : "Select a session to view details"}
							</Text>
						</Box>
					) : chatMode ? (
						<ChatPanel
							...
							<Box paddingX={1} flexDirection="column">
								...
								<Box>
									...
									<Box marginLeft={2}>
										...
										<Text color="gray">
											{formatRelativeTime(selectedSession.createTime)}
										</Text>
									</Box>
									<Box marginLeft={2}>
										<Text bold>Updated: </Text>
										<Text color="gray">
											{formatRelativeTime(selectedSession.updateTime)}
										</Text>
									</Box>
								</Box>
								<Box>
									<Text bold>Repo: </Text>
									<Text color="cyan">
										{extractRepo(selectedSession.sourceContext)}
									</Text>
								</Box>
								...
								{selectedSession.automationMode &&
									selectedSession.automationMode !== "NONE" && (
										<Box>
								...
								{selectedSession.outputs &&
									selectedSession.outputs.length > 0 && (
