...
const DEFAULT_TEMPLATES: Template[] = [
	{
		...
		prompt:
			"Fix the following bug in the repository: {{bug_description}}. The affected area is likely around {{affected_area}}.",
		variables: [
			{
	...
	},
	{
		...
		prompt:
			"Add comprehensive unit tests for the code in {{file_path}}. Use the existing testing framework and follow current patterns.",
		variables: [
			{
				name: "file_path",
	...
	},
	{
		...
		prompt:
			"Refactor the following code in {{file_path}} to improve {{improvement_goal}}. Make sure to maintain the same external behavior.",
		variables: [
			{
				name: "file_path",
...
class TemplatesManager {
	...
	delete(id: string): void {
		this.conf.delete(`templates.${id}` as string);
	}
	...
	reset(): void {
		this.conf.set(
			"templates",
			Object.fromEntries(DEFAULT_TEMPLATES.map((t) => [t.id, t])),
		);
	}
}
