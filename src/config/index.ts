...
class ConfigManager {
	...
	constructor() {
		this.conf = new Conf<CLIConfig>({
			...
			schema: schema as unknown as Schema<CLIConfig>,
		});
	}
	...
	async getOAuthClientCredentials(): Promise<{
		...
		try {
			const clientId = await getPassword(
				SERVICE_NAME,
				ACCOUNT_NAME_OAUTH_CLIENT_ID,
			);
			const clientSecret = await getPassword(
				SERVICE_NAME,
				ACCOUNT_NAME_OAUTH_CLIENT_SECRET,
			);
			return {
	...
	async setOAuthClientCredentials(
		clientId: string,
		clientSecret: string,
	): Promise<void> {
	...
	getApiEndpoint(): string {
		const envEndpoint =
			process.env.JULES_API_URL || process.env.JULES_API_ENDPOINT;
		if (envEndpoint) {
			return envEndpoint;
		}
		return (
			this.conf.get("apiEndpoint") || "https://jules.googleapis.com/v1alpha"
		);
	}
