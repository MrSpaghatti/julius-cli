...
function formatEventForDisplay(event: DaemonEvent): string {
	...
	const label =
		event.sessionId === "daemon" ? "DAEMON" : event.sessionId.slice(0, 12);
...
async function startBackgroundDaemon(): Promise<void> {
	...
	const child = fork(process.argv[1], ["daemon", "--foreground"], {
		detached: true,
	...
	const logFd = await import("node:fs/promises").then((m) =>
		m.open(EVENT_LOG, "a"),
	);
	...
	const cmd = new Command("daemon")
		...
		.action(
			async (
				...
				options: { json?: boolean; interval?: number },
			) => {
