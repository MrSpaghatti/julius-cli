...
export function createInteractiveCommand({
	commands,
	execute,
}: InteractiveCommandDependencies): Command {
	return new Command("interactive")
		...
		.action(async (options: { repo?: string }) => {
			...
			Output.info(
				chalk.gray(
					'Macros: "macro <name> <cmd...>" to define, "!<name>" to run.',
				),
			);
			if (currentRepo) {
			...
			rl.on("SIGINT", () => {
				if (isCommandRunning) {
					...
					Output.info("");
					const promptText = currentRepo
						? `julius-cli [${currentRepo}] > `
						: "julius-cli > ";
			...
			const executeCommand = async (parts: string[]) => {
				...
				try {
					...
					if (
						...
						commanderCodes.includes(errObj.code)
					) {
						// expected commander errors that should not crash the REPL
					} else {
						Output.error(
							chalk.red(
								`Error: ${errObj && typeof errObj.message === "string" ? errObj.message : String(err)}`,
							),
						);
					}
				} finally {
			...
			const handleRepoCommand = (trimmed: string): boolean => {
				...
				Output.info(
					chalk.gray(`Default repo set to: ${chalk.cyan(currentRepo)}`),
				);
				return true;
			};
			...
			const handleMacroCommand = (trimmed: string): boolean => {
				...
				Output.info(
					chalk.gray(`Macro '${macroName}' saved. Run with !${macroName}`),
				);
				return true;
			};
