...
export function createTemplatesCommands(): Command {
	const templatesCmd = new Command("templates").description(
		"Manage session prompt templates",
	);
	...
	templatesCmd
		...
		.option(
			"--format <format>",
			"Output format (json, pretty, table, quiet)",
			"pretty",
		)
		.action(async (options) => {
			...
			Output.log(
				formatOutput(
					{ kind: "templates", templates: allTemplates },
					options.format,
				),
			);
		});
	...
	templatesCmd
		...
		.action(async () => {
			...
			try {
				...
				Output.info(
					"Enter prompt (use {{var}} for variables). Press Enter twice to finish:",
				);
			...
			try {
				...
				if (!Array.isArray(imported)) {
					throw new InvalidArgsError(
						"Import file must contain an array of templates",
					);
				}
				let count = 0;
				for (const t of imported) {
					if (!t.id || !t.prompt) {
						Output.warn(
							chalk.yellow(
								`Skipping invalid template (missing id or prompt): ${t.id || "unknown"}`,
							),
						);
						continue;
					}
				...
				Output.info(
					chalk.green(`Successfully imported ${count} templates from ${file}`),
				);
			} catch (err: unknown) {
				throw new Error(
					`Failed to import templates: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		});
		...
		.action(async (id, vars, options) => {
			...
			template.variables?.forEach((variable) => {
				...
				if (variable.required && !value) {
					throw new InvalidArgsError(
						`Variable "${variable.name}" is required for template "${id}".`,
					);
				}
				if (value) {
