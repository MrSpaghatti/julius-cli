...
export function createConfigCommands(): Command {
	...
	configCmd
		...
		.argument(
			"<key>",
			"Configuration key (apiKey, apiEndpoint, defaultFormat, defaultPageSize, pollInterval, maxPollAttempts)",
		)
		...
		.action(async (key: string, value: string) => {
			...
			if (
				["defaultPageSize", "pollInterval", "maxPollAttempts"].includes(key)
			) {
			...
			if (
				key === "defaultFormat" &&
				!["json", "pretty", "quiet", "table"].includes(value)
			) {
				throw new InvalidArgsError(
					`Value for defaultFormat must be one of: json, pretty, quiet, table`,
				);
			}
	...
	configCmd
		...
		.action(
			async (
				key: string,
				options: { format: OutputFormat; show?: boolean },
			) => {
				let value;
				if (key === "apiKey") {
