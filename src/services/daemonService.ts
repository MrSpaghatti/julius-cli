...
import type { Session, SessionState } from "../api/types.js";
import {
	sendNotification,
...
export class DaemonService {
	...
	constructor(options: DaemonOptions = {}) {
		this.pollInterval =
			options.pollInterval ?? config.get("daemon")?.pollInterval ?? 30000;
		this.targets = options.notificationTargets ??
			config.get("daemon")?.notifications ?? ["system", "agent"];
		...
		try {
			...
			if (activities.length > 0) {
				...
				if (
					...
					latest.createTime > tracked.lastActivityTime
				) {
